...
team_check_detail = """select u.team, u.name, case when a.count>0 then '√' else 'x' end as `check`
from tprj_user u left join tprj_activity a on u.mixin_id = a.mixin_id and a.type='打卡' and a.date='{date}'
where u.team='{team}'
ORDER BY a.count"""

sale_data = '''select u.team, u.name, sum(count) count
from tprj_user u inner join tprj_activity a on u.mixin_id = a.mixin_id and a.type='开单'
GROUP BY a.mixin_id'''