...
class SaleRecord:
    ...
    def show_team_score(self, rtype='txt'):
        data = self.db.qj(sql.team_score_show)
        if rtype == 'dict':
            return self.sqlRowsToDict(data)
        result = []
        line = '\t'.join(data[0].keys()) + "\n"
...
def main():
    ...
    with open(filename, 'w', encoding='utf-8') as r:
        ...
        r.writelines(sale.show_check_detail())
        r.writelines(sale.show_team_score())
...
def test():
    ...
    data = sale.show_check_rate(rtype='dict')
